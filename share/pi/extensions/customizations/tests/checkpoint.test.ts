...
test("restore: modified untracked file content is restored", () =>
  withTestRepo(async (dir, root) => {
    ...
    assert(
      (await readFile(join(dir, "untracked.txt"), "utf-8")) ===
        "original content",
...
test("restore: modified file content is restored", () =>
  withTestRepo(async (dir, root) => {
    ...
    // Verify content
    assert(
      (await readFile(join(dir, "initial.txt"), "utf-8")) ===
        "modified content",
...
test("restore: file in HEAD but not in worktree is removed", () =>
  withTestRepo(async (dir, root) => {
    ...
    assert(
      ...
      "Setup: initial.txt should exist",
    );
...
test("restore: complex state with mixed staged/unstaged/untracked", () =>
  withTestRepo(async (dir, root) => {
    ...
    assert(
      indexFiles.includes("initial.txt"),
      "initial.txt should be in index",
    );
    assert(
      indexFiles.includes("staged_new.txt"),
      "staged_new.txt should be in index",
    );
    assert(
...
test("isLargeFile: returns false for files exactly at limit", () =>
  withTestRepo(async (dir, root) => {
    ...
    assert(
      !isLargeFile(root, "exact.bin"),
      "File exactly at 10 MiB should not be large",
    );
  }));
...
test("isLargeDirectory: returns false for small directories", () =>
  withTestRepo(async (dir, root) => {
    ...
    assert(
      !isLargeDirectory(root, "smalldir"),
      "Directory with 10 files should not be large",
    );
  }));
...
test("isLargeDirectory: returns true for directories > 200 files", () =>
  withTestRepo(async (dir, root) => {
    ...
    assert(
      isLargeDirectory(root, "largedir"),
      "Directory with 201 files should be large",
    );
  }));
...
test("restore: tracked directory with many tracked files is not treated as large due to a small number of untracked files", () =>
  withTestRepo(async (dir, root) => {
