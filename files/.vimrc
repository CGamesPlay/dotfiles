...
if version >= 703
  ...
  set number
  " I prefer this, but it slows the UI too much to be useful.
  "set relativenumber
end
...
" Don't update the screen while running macros
set lazyredraw
...
augroup os_appearance " {{{
  au!
  " Switch to a dark background in Dark Mode.
  if exists('v:os_appearance')
    function! s:SyncOSAppearance()
...
" Quick way to toggle prettier write_on_save
command! PrettierDisable let g:prettier#write_on_save = 0
command! PrettierEnable let g:prettier#write_on_save = 1
...
augroup prettier " {{{
  ...
  autocmd BufWritePre *.js,*.json,*.css,*.scss,*.less,*.graphql,*.ts,*.tsx if g:prettier#write_on_save | Prettier | endif
augroup END
...
augroup ft_go " {{{
  ...
  au FileType go setlocal nolist noexpandtab sw=4 sts=4 ts=4
augroup END
